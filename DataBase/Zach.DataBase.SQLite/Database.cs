...
namespace Zach.DataBase.SQLite
{
    /// <summary>
    /// 数据库操作类
    /// </summary>
    public class Database : IDatabase
    {
        ...
        public int Commit()
        {
            ...
            {
                if (ex.InnerException != null && ex.InnerException.InnerException is SQLiteException)
                {
        ...
        public IEnumerable<T> QueryByProc<T>(string procName) where T : class
        {
            if (dbTransaction == null)
            {
                return dbcontext.Database.Connection.Query<T>(procName, null, null, true, null, CommandType.StoredProcedure);
            }
            else
            {
        ...
        public IEnumerable<T> QueryByProc<T>(string procName, object dbParameter) where T : class
        {
            ...
            if (dbTransaction == null)
            {
                return dbcontext.Database.Connection.Query<T>(procName, dbParameter, null,true, null, CommandType.StoredProcedure);
            }
            else
            {
        ...
        public DataTable FindTableByProc(string procName, object dbParameter)
        {
            if (dbTransaction == null)
            {
                var IDataReader = dbcontext.Database.Connection.ExecuteReader(procName, dbParameter, null, null, CommandType.StoredProcedure);
                return SqlHelper.IDataReaderToDataTable(IDataReader);

            }
            else
            {
                var IDataReader = dbTransaction.Connection.ExecuteReader(procName, dbParameter, dbTransaction, null, CommandType.StoredProcedure);
                return SqlHelper.IDataReaderToDataTable(IDataReader);
